...
public:
	...
	void print_srv_and_PFT(std::vector< std::shared_ptr<CPlant> > & PlantList); // prints PFT data
	void print_ind(std::vector< std::shared_ptr<CPlant> > & PlantList); // prints individual data
	void print_meta();
